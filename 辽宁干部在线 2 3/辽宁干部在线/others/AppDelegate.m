...
-(void)addRequest:(NSString *)url withInfo:(NSDictionary *)info
{
    if (![[_asiDic allValues] containsObject:url]) {
        ...
        [_asiDic setObject:url forKey:[NSString stringWithFormat:@"%d",asi.tag]];
        [_downDic setObject:info forKey:[NSString stringWithFormat:@"%d",asi.tag]];
...
#warning 4-29
//4-29做，下载完毕以后，将userdefault修改，然后将字典移除成功的请求，另，加model，将dealloc之前的progress保存下来

-(void)requestFailed:(ASIHTTPRequest *)request
{
    [_asiDic removeObjectForKey:[NSString stringWithFormat:@"%d",request.tag]];
    
    NSLog(@"%@",request.error);
    NSLog(@"requestFailed:%d",[[_asiDic allKeys] count]);
    NSLog(@"requestFailed:%d",_queue.operationCount);
}
...
-(void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    [arr removeObject:[_asiDic objectForKey:[NSString stringWithFormat:@"%d",request.tag]]];
    [dic removeObjectForKey:[_asiDic objectForKey:[NSString stringWithFormat:@"%d",request.tag]]];
    [dicDown addObject:[_downDic objectForKey:[NSString stringWithFormat:@"%d",request.tag]]];
    [_downDic removeObjectForKey:[NSString stringWithFormat:@"%d",request.tag]];
    [_asiDic removeObjectForKey:[NSString stringWithFormat:@"%d",request.tag]];
